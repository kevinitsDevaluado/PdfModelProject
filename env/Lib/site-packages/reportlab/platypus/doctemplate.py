...
the page templates for the next page (which by default is the one being used
...
class ActionFlowable(Flowable):
    '''This Flowable is never drawn, it can be used for data driven controls
       For example to change a page templates (from one column to two, for example)
...
class NextPageTemplate(ActionFlowable):
    locChanger = 1                  #we cause a frame or page change
    """When you get to the next page, use the templates specified (change to two column, for example)  """
...
class PageTemplate:
    ...
    def checkPageSize(self,canv,doc):
        """This gets called by the templates framework
        If canv size != templates size then the canv size is set to
        the templates size or if that's not available to the
        doc size.
        """
...
class BaseDocTemplate:
    """
    First attempt at defining a document templates class.
    ...
    3)  Each page templates has one or more frames.
    ...
    since it builds a document using the page templates.

    Each document templates builds exactly one document into a file specified
    ...
    - pageTemplates: A list of templates.  Must be nonempty.  Names
      ...
      templates should have the same name.  For example you might want one templates
    ...
    def __init__(self, filename, **kw):
        """create a document templates bound to a filename (see class documentation for keyword arguments)"""
    ...
    def handle_nextPageTemplate(self,pt):
        '''On endPage change to the page templates with name or index pt'''
        if isinstance(pt,strTypes):
            ...
            for t in self.pageTemplates:
                if t.id == pt:
                    ...
                    return
            raise ValueError("can't find templates('%s')"%pt)
        elif isinstance(pt,int):
            if hasattr(self, '_nextPageTemplateCycle'): del self._nextPageTemplateCycle
            self._nextPageTemplateIndex = pt
        elif isSeq(pt):
            #used for alternating left/right pages
            #collect the refs to the templates objects, complain if any are bad
            c = PTCycle()
            for ptn in pt:
                found = 0
                ...
                for t in self.pageTemplates:
                    if t.id == ptn:
                        ...
                        found = 1
                if not found:
                    raise ValueError("Cannot find page templates called %s" % ptn)
            if not c:
            ...
            if frame.add(f, canv, trySplit=self.allowSplitting):
                ...
                if n:
                    ...
                    if hasattr(f,'_postponed'):
                        ident = "Flowable %s%s too large on page %d in frame %r%s of templates %r" % \
...
class SimpleDocTemplate(BaseDocTemplate):
    """A special case document templates that will handle many simple documents.
    ...
    def handle_pageBegin(self):
        '''override base method to add a change of page templates after the firstpage.
        '''
