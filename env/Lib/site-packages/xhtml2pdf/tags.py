...
class pisaTagPDFTEMPLATE(pisaTag):
    """
    <pdf:templates name="" static box="" >
        <pdf:frame...>
    </pdf:templates>
    """

    def start(self, c):
        deprecation("pdf:templates")
        attrs = self.attr
        name = attrs["name"]
        c.frameList = []
        c.frameStaticList = []
        if name in c.templateList:
            log.warning(c.warning("templates '%s' has already been defined", name))

    def end(self, c):
        attrs = self.attr
        name = attrs["name"]
        if len(c.frameList) <= 0:
            log.warning(c.warning("missing frame definitions for templates"))
